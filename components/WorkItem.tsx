...
export function WorkItem({ project, index }: WorkItemProps) {
  ...
  return (
    <article
      className={`border-t border-[var(--line)] py-10 transition-colors duration-150 ${
        project.highlight ? "bg-[var(--paper-card)] -mx-4 px-4 sm:-mx-6 sm:px-6 border-b" : ""
      }`}
    >
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
        {/* Left Column: Index & Metadata */}
        <div className="lg:col-span-3 flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <span className="font-mono text-xs font-medium text-[var(--accent)] tracking-widest">
              0{index + 1}
            </span>
          ...
          {project.highlight && (
            <div className="mt-2 inline-flex items-center gap-1.5 self-start px-2 py-0.5 border border-[var(--accent-border)] bg-[var(--accent-subtle)] text-[var(--accent)] text-[11px] font-mono">
        ...
        {/* Right Column: Title, Strong sentence, Details */}
        <div className="lg:col-span-9 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
            <h3 className="font-serif text-2xl sm:text-3xl text-[var(--ink)] tracking-tight font-normal">
              {project.title}
            </h3>
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {project.tags.map((tag) => (
              <span
                key={tag}
                className="font-mono text-xs px-2.5 py-1 border border-[var(--line)] text-[var(--ink-secondary)] bg-[var(--paper)]"
              >
                {tag}
              </span>
            ))}
          ...
          <div className="mt-3 pt-3 border-t border-[var(--line-faint)]">
            <button
              type="button"
              onClick={() => setExpanded(!expanded)}
              className="inline-flex items-center gap-2 text-xs font-mono text-[var(--ink)] hover:text-[var(--accent)] focus:outline-none transition-colors"
            ...
            {expanded && (
              <div className="mt-4 pt-4 border-t border-[var(--line)] grid grid-cols-1 md:grid-cols-2 gap-6 bg-[var(--paper)] p-5 border border-[var(--line)]">
                <div>
                ...
                {project.metrics && (
                  <div className="border-t md:border-t-0 md:border-l border-[var(--line)] pt-4 md:pt-0 md:pl-6 flex flex-col justify-between">
                    <div>
                      <h4 className="font-mono text-xs uppercase tracking-wider text-[var(--muted)] mb-3">
